...
import logging
import traceback
from app.utils.constvalue import astro_app_publickey,astro_alipay_publickey,astro_app_secretkey,fish_serverurl,astro_appid
...
astro_alipay_client_config = AlipayClientConfig()
astro_alipay_client_config.server_url = 'https://openapi.alipay.com/gateway.do'
astro_alipay_client_config.app_id = astro_appid
astro_alipay_client_config.app_private_key =astro_app_secretkey
astro_alipay_client_config.alipay_public_key = astro_alipay_publickey

astro_client = DefaultAlipayClient(alipay_client_config=astro_alipay_client_config, logger=logger)


@alipay.route("/astro/pay")
def astropay():
    ...
    result = {}
    try:
        response_content = astro_client.execute(request_ali)
...
def astro_page_pay():
    ...
    response = astro_client.page_execute(request, http_method="GET")
    print("alipay.trade.page.pay response:" + response)


@alipay.route("/astro/app/pay")
def astro_app_pay():
    ...
    ordername = request.args.get("ordername", "天文观星指南阳春三月特惠(终身会员)")
    if  type == "1":
        model.total_amount = "33.00"
        if amount:
        ...
        if amount:
            model.total_amount = amount
        model.subject = "天文观星指南暑假特惠(终身会员)"
    else:
        model.total_amount = "98.00"
        if amount:
            model.total_amount =amount
        model.subject = '天文观星指南终身会员'
    ...
    response = astro_client.sdk_execute(request_ali)
...
@alipay.route("/astro/app/pay/v2")
def astro_app_pay_v2():
    model = AlipayTradeAppPayModel()
    type = request.args.get("type", "1")
    type = str(type)
    out_trade_no = request.args.get("out_trade_no","151671406202021031011")
    ...
    if  type == "1":
        ...
        if amount:
            model.total_amount = amount
        model.subject = "天文观星指南春日出游特惠(终身会员)"
    else:
