...
import logging
import traceback
from app.utils.constvalue import fish_alipay_publickey,fish_app_secretkey,fish_appid,fish_serverurl
...
fish_alipay_client_config = AlipayClientConfig()
fish_alipay_client_config.server_url = 'https://openapi.alipay.com/gateway.do'
fish_alipay_client_config.app_id = fish_appid
fish_alipay_client_config.app_private_key =fish_app_secretkey
fish_alipay_client_config.alipay_public_key = fish_alipay_publickey

fish_client = DefaultAlipayClient(alipay_client_config=fish_alipay_client_config, logger=logger)


@alipay.route("/fish/pay")
def fishpay():
    ...
    result = {}
    try:
        response_content = fish_client.execute(request_ali)
...
def fish_page_pay():
    ...
    response = fish_client.page_execute(request, http_method="GET")
    print("alipay.trade.page.pay response:" + response)


@alipay.route("/fish/app/pay")
def fish_app_pay():
    ...
    ordername = request.args.get("ordername","2023年终特惠(终身会员)")
    if  type == "1":
        ...
        if amount:
            model.total_amount = amount
        model.subject = ordername.encode('utf8')
    else:
    ...
    request_ali.notify_url = "https://www.xunquan.shop/api/v3.0/alipay/app/pay/notify"
