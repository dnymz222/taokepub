...
@api3.route("/solareclipse/centry/ten")
def solareclipsecenteryten():
    for i in range(20,30):
        solareclipsecentery(i)
...
def solareclipseOval(solarEclipseItem: SolarEclipse,century: str):
    ...
    plt.text(0.02, 0.97, solarEclipseItem.eclipsectypename() + " (Mag:%.3f)"%solarEclipseItem.magnitude, fontsize=30,color="#F38C30",transform=ax.transAxes)

    # otime_str = dt_utc.strftime("%Y-%m-%d %H:%M")

    otime_str = "%d %s %02d %02d:%02d" % (
    dt_utc.year, monthlist[dt_utc.month - 1], dt_utc.day, dt_utc.hour, dt_utc.minute)

    plt.text(0.69, 0.97,   otime_str + " UTC", fontsize=29,color="#F38C30",transform=ax.transAxes)
